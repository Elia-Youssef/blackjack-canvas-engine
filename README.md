...
project, ahead of that second block; the two python gates stay workspace-level commands.

**`npm run verify` is the fast chain, and CI requires more than it runs.** The merge gate adds the
five measurement reports (`report:bundle-size`, `report:contrast`, `report:touch-targets`,
`report:perf`, `report:lighthouse`) to the same job, `report:memory` in a thirty-minute soak job of
its own, and the containerised visual-baseline run. `npm run verify:reports` is the local half of
that: the four browser-driven measurements that finish in minutes, in CI's order. The memory soak,
the Lighthouse run and the visual baselines stay CI-only, so "green locally" means the fast chain
plus those four and not the whole merge gate. `npm run report:all` runs all six reports, and it runs
every one of them before it fails, so a breach in the third still leaves the fourth, fifth and sixth
artifacts on disk.

Once per machine, the browser gate needs its engines:
...
| `npm run verify:reports` | The four measurement reports CI runs beside the gates above and that finish in minutes: the bundle ceilings, the contrast audit, the touch-target sweep and the frame-timing run. Not chained into `npm run verify`, which is the fast chain |
| `npm run report:all` | All six reports, every one of them run before the command fails, so a breach in one still leaves the others' artifacts. `report:memory` is a thirty-minute soak and `report:lighthouse` installs its own runner, which is why neither is in `verify:reports` |
